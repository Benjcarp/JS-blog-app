...
cancelBtn.addEventListener("click", async () => {
  const answer = await (0,_assets_javascript_modal__WEBPACK_IMPORTED_MODULE_1__.openModal)("attention, en annulant vous perdrez votre article, confirmez vous votre annulation");
  if (answer) {
...
form.addEventListener("submit", async event => {
  ...
  if (formIsValide(data)) {
    try {
      ...
      let response;
      if (articleId) {
        response = await fetch(`https://restapi.fr/api/dwwm_benjamin/${articleId}`, {
          method: "PATCH",
          headers: {
