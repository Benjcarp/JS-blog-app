...
const displayArticles = () => {
  ...
  deleteBtn.forEach(button => {
    button.addEventListener("click", async event => {
      ...
      window.scrollTo(0, 0);
      const answer = await (0,_assets_javascript_modal__WEBPACK_IMPORTED_MODULE_1__.openModal)("etes vous sur de vouloir supprimer votre article ?");
      if (answer) {
        try {
...
const fetchArticles = async () => {
  ...
  try {
    const response = await fetch(`https://restapi.fr/api/dwwm_benjamin?sort=createdAt:${sortedBy}`);
